    ...
    <nav aria-label="Page navigation example">
      ...
      <c:choose>
        <c:when test="${pageNum == 1}">
          <li class="page-item"><a class="page-link" href="SearchResult">Previous</a></li>
        </c:when>
        <c:otherwise>
          <li class="page-item"><a class="page-link" href="SearchResult?search_word=${searchWord}&page_num=${pageNum - 1}">>Previous</a></li>
        </c:otherwise>
        </c:choose>

      <c:forEach begin="${(pageNum - 5) > 0 ? pageNum - 5 : 1}" end="${(pageNum + 5) > pageMax ? pageMax : pageNum + 5}" step="1" varStatus="status">
        <li class="page-item" <c:if test="${pageNum == status.index }"> class="active" </c:if>><a class="page-link" href="SearchResult?search_word=${searchWord}&page_num=${status.index}">${status.index}</a></li>
      </c:forEach>

      <c:choose>
        <c:when test="${pageNum == pageMax || pageMax == 0}">
          <li class="page-item"><a class="page-link" href="SearchResult.html">Next</a></li>
        </c:when>
        <c:otherwise>
          <li class="page-item"><a href="SearchResult?search_word=${searchWord}&page_num=${pageNum + 1}">NEXT</a></li>
        </c:otherwise>
      </c:choose>
